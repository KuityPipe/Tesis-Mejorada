...
- `codigo/biometria/reconocimiento_facial/probando_face_recognition.py` was rewritten from the byte-dump it used to be and is now wired into Django (`/rostro/`, see Data Flow) with a real place to store the reference encoding (`Usuario.encoding_facial`, migration `0024`) — the piece that was missing before. `rostro.html` also got a real in-browser webcam capture widget (`_captura_camara.html`, `getUserMedia` + `<canvas>` → fills the same `<input type="file">` the server already validated, falls back to the normal file picker if the browser/user denies camera access). What's still unverified: `opencv-python`/`face_recognition` aren't installed in this dev environment (heavy, `dlib` compiles natively — same posture as before), so the actual face-detection/comparison code has only run through mocked tests (`ReconocimientoFacialTests`), never against a real photo. **Installing them was attempted and failed**: PyPI has no prebuilt `dlib` wheel for Python 3.14 yet, this machine has neither a C++ toolchain nor winget/choco, and a from-scratch Visual Studio Build Tools install (bootstrapper only, `--add Microsoft.VisualStudio.Workload.VCTools`) hit `NameResolutionFailure` mid-download over a flaky mobile connection and aborted with "Bootstrapper failed with known error" (exit code 5008) — not a permissions or disk-space problem, just a bad network mid-download. Retry this step (or find an environment with a stable connection) before trying to exercise real face detection; once a C++ toolchain exists, `pip install opencv-python==4.9.0.80 face_recognition==1.3.0` per `requirements.txt` should be enough. If a real photo eventually produces zero or multiple face detections, unexpected lighting/angle behavior, or a `tolerance=0.6` threshold that's too strict/loose, none of that is caught yet either.
