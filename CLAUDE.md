...
python manage.py test KeyServApp   # 116 tests, all passing
...
codigo/biometria/reconocimiento_facial/
  └── probando_face_recognition.py  cargar_rostro_conocido()/verificar_rostro_en_imagen() (single-image compare, used by the real web flow) + verificar_rostro() (live webcam loop, manual/local testing only, never called from Django) — opencv-python/face_recognition/dlib are installed and importable in this environment now, but still untested against a real photo/webcam (no webcam or sample face photo available here, see Known Issues)
```
...
**Facial recognition (wired in this session, still unverified against real hardware):** `/rostro/` (`rostro_view`) shows a register-or-verify screen depending on whether `Usuario.encoding_facial` (migration `0024`, `JSONField` — new, this is what was missing before) is set. `registro_rostro_view` (`POST /rostro/registrar/`) uploads a reference photo (validated with the same `validators.validar_imagen` as everything else) and stores its 128-float encoding via `biometria.calcular_encoding_facial`. `verificacion_facial_view` (`POST /rostro/verificar/`) uploads a second photo and compares it against the stored encoding via `biometria.verificar_rostro_usuario`, setting the same `Usuario.verificado_biometricamente` flag the fingerprint flow uses (RF001 treats fingerprint/face/Clave Única as alternative methods, not all-required, so sharing one flag is intentional, not a shortcut). Unlike the fingerprint flow, this one has a real reference to compare against — but it's still only exercised through `ReconocimientoFacialTests`, which mock `biometria.calcular_encoding_facial`/`verificar_rostro_usuario` at the view level (`opencv-python`/`face_recognition`/`dlib` are installed and working in this environment now, see Known Issues, but no real photo has been run through the pipeline yet). The backend never opens a webcam itself — `verificar_rostro_en_imagen()` (`probando_face_recognition.py`) compares a single uploaded photo, matching how the fingerprint flow already worked and how the original skeleton's own docstring said this should be done (capture happens in the client's browser, not the server). `rostro.html` includes a reusable `_captura_camara.html` widget (plain `getUserMedia`/`<canvas>`, no library) that lets the browser take the photo directly instead of only picking an existing file — see Known Issues for why this still hasn't been exercised against real face detection.
...
- `codigo/biometria/reconocimiento_facial/probando_face_recognition.py` was rewritten from the byte-dump it used to be and is now wired into Django (`/rostro/`, see Data Flow) with a real place to store the reference encoding (`Usuario.encoding_facial`, migration `0024`) — the piece that was missing before. `rostro.html` also got a real in-browser webcam capture widget (`_captura_camara.html`, `getUserMedia` + `<canvas>` → fills the same `<input type="file">` the server already validated, falls back to the normal file picker if the browser/user denies camera access). **`opencv-python`/`face_recognition`/`dlib` are now installed and importable** in `venv_tesis` (a first attempt failed — see below — but a retry succeeded once the network was stable). Getting there needed three fixes beyond just retrying the download:
  ...
  With all three in place, `manage.py test KeyServApp` (116 tests) passes clean — including one real bug this surfaced: `biometria.calcular_encoding_facial`/`verificar_rostro_usuario` only caught `ImportError`/`ValueError` around the *entire* try block (import + call together), so a missing/unreadable image file raised an uncaught `FileNotFoundError`/`OSError` instead of returning `None` like their own docstrings promised (this only became reachable once the real module could actually be imported — previously `ImportError` always fired first and masked it). Fixed by splitting the import and the call into separate `try` blocks, matching the pattern `procesar_huella_dactilar` already used in the same file.

  **What's still unverified**: no real photo or webcam has been run through the pipeline in this environment — `ReconocimientoFacialTests` still mocks `biometria.calcular_encoding_facial`/`verificar_rostro_usuario` at the view level rather than exercising real face detection. If a real photo eventually produces zero or multiple face detections, unexpected lighting/angle behavior, or a `tolerance=0.6` threshold that's too strict/loose, none of that is caught yet.
