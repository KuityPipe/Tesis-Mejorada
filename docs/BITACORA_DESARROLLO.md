...
| 68 | 2026-08-16 14:15 | `d1ccf09` | Búsqueda por geolocalización (radio en km, solo Ionic) + mini-mapa/link a Google Maps en `catalogo/detalle` — `Comuna.latitud`/`longitud` (migraciones 0026/0027, 9 comunas RM geocodificadas), `_anotar_distancia_km` (Haversine vía ORM) en `PublicacionListView`, `core/ubicacion.ts` (`@capacitor/geolocation`) + selector de radio en `catalogo.page`, `latitud`/`longitud` en `ProveedorSerializer` + embed de Google Maps sin API key en `catalogo/detalle`. 3 bugs reales encontrados y corregidos en el camino (`LEAST()` de Postgres ignorando NULL, `requestPermissions()` no implementado en la versión web de `@capacitor/geolocation`, timeout de `enableHighAccuracy: false` en el emulador Android). Verificado en vivo en `:8100`, y en el emulador Android real (`KeyServ_Test2`) con `adb emu geo fix` simulando el GPS. 269 tests backend + 59 frontend en verde. | ~1 h |
| 69 | 2026-08-16 15:57 | `dc9e650` | Deja lista la configuración de deploy para el demo desplegado — `render.yaml`/`render_build.sh`/`requirements-render.txt` (backend + Postgres en Render), `netlify.toml`/`environment.prod.ts` (frontend en Netlify), comando nuevo `sembrar_datos_demo` (idempotente, 8 proveedores + 3 clientes + 4 contrataciones), soporte para `DATABASE_URL`/whitenoise/`CSRF_TRUSTED_ORIGINS` en `settings.py`. Encontró y corrigió un bug real: `loaddata` corrido después de la migración 0027 pisaba las coordenadas de las comunas con NULL. 269 tests backend + 59 frontend en verde, `collectstatic` y build de producción de Ionic probados de verdad. | ~40 min |
| 70 | 2026-08-16 16:21 | `6f0a21c` | Demo desplegado de verdad, con el usuario creando las cuentas — 3 bugs reales encontrados y corregidos en el propio dashboard (no en código): Netlify tomaba `master` como rama de producción por defecto en vez de `api-ionic-migration` (build fallaba con "Base directory does not exist"), `CORS_ALLOWED_ORIGINS`/`IONIC_FRONTEND_URL` en Render seguían con el placeholder de `localhost` (bloqueaba todo fetch), y el sitio de Netlify nacía privado ("Private by default"). Verificado en vivo en el sitio real: catálogo con las 8 publicaciones demo, login real, dashboard con la contratación sembrada. `keyserv.netlify.app` (frontend) + `keyserv-api.onrender.com` (backend), URL y credenciales agregadas al README. | ~25 min |
| 71 | 2026-08-16 16:36 | `98d7f9d` | Fase 7: endpoint de refresh token + rotación — `POST /api/auth/refresh/` (`RefreshView`) y `POST /api/auth/logout/` (`LogoutView`, revoca `TokenSesion` del lado del servidor de verdad, antes `Auth.logout()` solo borraba local). `Auth.refrescarToken()` (Ionic, con `shareReplay(1)` para no disparar dos refresh en paralelo) + `authInterceptor` reintenta sola una request que volvió con 401. Verificado en vivo contra el backend real corriendo (no solo tests): login → refresh rota el token → reusar el viejo da 401 → logout → refrescar después del logout también da 401. 9 tests backend + 10 tests frontend nuevos, 278 tests backend + 67 frontend en verde. | ~35 min |
| 72 | 2026-08-16 17:04 | `98baa67` | Build filters en Render/Netlify — pedido explícito del usuario tras ver deploys "Canceled" en los dashboards (varios commits seguidos, cada uno disparando un rebuild completo de ~50s en cada lado). `render.yaml`: `buildFilter.paths` acotado a `codigo/backend/django/**`/`requirements-render.txt`/`render_build.sh`. `netlify.toml`: `ignore = "git diff --quiet $CACHED_COMMIT_REF $COMMIT_REF ."` (relativo al Base Directory). Verificado en vivo en ambos dashboards: el deploy de este mismo commit corrió normal (toca `render.yaml`/`netlify.toml`, dentro de los paths filtrados), y el siguiente commit (solo esta bitácora) no debería disparar ningún build en ninguno de los dos lados — a confirmar en el próximo checkpoint. | ~15 min |
| 73 | 2026-08-16 21:50 | `2378ceb` | Rediseño del chat embebido en `contratacion/detalle` — pedido explícito del usuario ("se ve muy soso"): burbujas propia/ajena, scroll propio acotado a la sección, fotos adjuntas (`Mensaje.imagen`, migración `0028`) y picker de 16 emojis frecuentes (sin stickers, decisión explícita del usuario). Dos bugs reales encontrados en la verificación en vivo, ninguno detectado por los tests: la migración `0028` nunca se había aplicado contra el Postgres de desarrollo (500 al primer envío real), y las fotos adjuntas se veían como ícono roto porque un `<img>` no puede mandar el header `Authorization` que exige el endpoint privado — corregido descargándolas como blob autenticado (`cargarImagenSegura()`). Verificado en vivo contra `:8100` real: texto (botón y Enter), emoji insertado, y foto real adjunta, las tres como burbuja propia con scroll acotado. 12 tests backend + 9 frontend nuevos, 289 backend + 73 frontend en verde. | ~1 h 10 min |
| 74 | 2026-08-16 22:05 | `a260f6a` | "Exportar chat" — endpoint API que faltaba: `GET /api/contrataciones/<id>/mensajes/exportar/` (`ContratacionMensajesExportarView`), equivalente de `conversacion_exportar_view`. El formateo del `.txt` se sacó a `_texto_exportar_conversacion` (views.py), compartida por el template y la API — de paso corrigió un bug real que ya existía en el template desde que se agregó `Mensaje.imagen`: un mensaje solo con foto imprimía "None" en el backup en vez de "[foto]". Botón "Exportar chat" nuevo en `contratacion/detalle`, descarga el `.txt` como blob autenticado (mismo motivo que la foto de un mensaje). Verificado en vivo contra `:8100` real: descarga real a la carpeta de Descargas, con "[foto]" correcto en el mensaje con imagen. 5 tests backend + 3 frontend nuevos, 294 backend + 76 frontend en verde. | ~15 min |
| 75 | 2026-08-16 23:10 | `5920de1` | Retira `huella.html` (demo legacy) del sitio Django — pedido explícito del usuario, ya superado por biometría nativa + reconocimiento facial. Sacados: `huella_view`/`verificacion_huella_view`, las dos rutas, `huella.html`, `biometria.procesar_huella_dactilar()`, y sus dos clases de test dedicadas. Actualizados los dos links que apuntaban ahí (`perfil.html`, `registroinicio.html`) y el copy de marketing que mencionaba "huella dactilar" (`Acercadeenosotros.html`/`paginicio.html`/`preferencias_cuenta.html`), ahora dicen "reconocimiento facial". `IMAGEN_HUELLA.py` (el pipeline en sí) queda intacto, standalone. Verificado en vivo: `/huella/` da 404, `/registro/`/`/` renderizan sin la copy vieja. 290 tests backend en verde. | ~35 min |
| 76 | 2026-08-16 23:55 | `(pendiente)` | Tests E2E web con Playwright — pedido explícito del usuario, alcance acotado a solo web (Appium nativo queda para una sesión aparte, decisión explícita). `codigo/frontend/ionic/e2e/` (`playwright.config.ts` + login/catálogo/chat) corre contra la app real (Chromium real + `ng serve` + API Django real con Postgres y datos demo), no mockea nada. Job nuevo `e2e` en `.github/workflows/tests.yml` (su propio Postgres, migra+siembra, levanta el backend en segundo plano con health-check). Dos ajustes reales no anticipados: los componentes Ionic exponen el mismo placeholder en el custom element Y su `<input>` de shadow DOM (locators por placeholder solo dan "strict mode violation"), y el filtro del catálogo dispara en `(ionChange)` (blur/Enter), no en cada tecleo. Verificado en vivo contra `:8000`/`:8100` reales: 7/7 en verde, repetido dos veces (14/14) sin flaky. | ~45 min |

**Tiempo activo estimado hasta el checkpoint #10** (sumando solo los intervalos cortos dentro del mismo día, sin las pausas entre días): ~9 h 45 min. **Hasta el checkpoint #13**: ~13 h 30 min. **Hasta el checkpoint #17**: ~15 h 41 min. **Hasta el checkpoint #19**: ~16 h 33 min. **Hasta el checkpoint #21**: ~17 h 04 min. **Hasta el checkpoint #22**: ~17 h 47 min. **Hasta el checkpoint #27**: ~25 h 35 min. **Hasta el checkpoint #34**: ~27 h 16 min. **Hasta el checkpoint #40** (cierre de Fases 3-6 de la migración API/Ionic): ~33 h 08 min. **Hasta el checkpoint #50** (cierre de la sesión nocturna + plan de portafolio): ~36 h 27 min. **Hasta el checkpoint #63** (cierre de la sesión de portafolio + pulido visual, 15 ago): ~40 h 09 min. **Hasta el checkpoint #64** (retema del panel de /admin/): ~41 h 38 min. **Hasta el checkpoint #65** (ancho del dashboard + alcance del Moderador acotado): ~42 h 09 min. **Hasta el checkpoint #66** (microinteracciones + rediseño del header): ~43 h 38 min. **Hasta el checkpoint #67** (registro pulido + prueba E2E Android + quitar tipo_cuenta): ~45 h 11 min. **Hasta el checkpoint #70** (geolocalización + config de deploy + demo desplegado en vivo, sesión del 16 ago, aparte de la anterior por la pausa larga entre días): ~2 h 05 min.
